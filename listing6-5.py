...
for planetsectors in range(1, 26):
  GAME_MAP.append(["The dusy planet surface", 13, 13, True, True])
...
for key, room_scenery_list in scenery.items():
  for scenery_item_list in room_scenery_list:
    checksum += (scenery_itsm_list[0] * key
                 + scenery_item_list[1] * (key + 1)
                 + scenery_item_list[2] * (key + 2))
    check_counter += 1
print(check_counter, checksum)
assert check_count == 161, "Expected 161 scenery items"
...
for room in range(1,26): # add random scenery in planet locations
  if room !=13: # skip room 13
    scenery_item  = random.choice([16, 28, 19, 30])
...
def generate_map():
  # This function makes tha map for the current room,
  # using room data, scenery data and prop data.
  
  if current_room in scenery: # checks if there's an entry for the current room in scenery dict
    for this_scenery in scenery[current_room]: # cycles through scenery items for the room and copies them into a list called this_scenery
  ...
  global topo_left_x, top_left_y, wall_transparency_frame
  ...
  #add doorways, putting them in the middle of the width and height of the room
  middle_row = int(room_height / 2)
  middle_column = int(room_width / 2)
  
  if room_data[4]: # if room has right exit
    # places what tile type to use for the door at room_map list
  ...
  if room_data[3]: # if exit at top of current room
  ...
  if current_room <= MAP_SIZE - MAP_WIDTH: # if room is not on bottom row
    ...
    # if room below has top exit, add exit at bottom of this room
    if room_below[3]:
